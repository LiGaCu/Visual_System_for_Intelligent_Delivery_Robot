...
int main(int argc, char** argv)
{
	...
	cap.open("/home/jiatong/1.avi");//RoadVideo.mp4");//
	int FramesTotal = cap.get(CV_CAP_PROP_FRAME_COUNT);
	char textshow[50];
	...
	namedWindow("ROI Video", CV_WINDOW_AUTOSIZE);
	namedWindow("Canny Video", CV_WINDOW_AUTOSIZE);
	namedWindow("Multiline Video", CV_WINDOW_AUTOSIZE);
	//	namedWindow("S", CV_WINDOW_NORMAL);
	createTrackbar("Frame", "Source Video", &realtime_frame, FramesTotal - 1, 0);
	createTrackbar("loThreshold", "Canny Video", &loThreshold, 255, 0);
	createTrackbar("upThreshold", "Canny Video", &upThreshold, 255, 0);


	//VideoWriter writer("E:\\毕业设计\\路面视频\\ProcessedVideo3_2.avi", CV_FOURCC('D', 'I', 'V', 'X'), 30, dsize);


	while (runstill)
	{
		keyscan();
		cap.set(CV_CAP_PROP_POS_FRAMES, realtime_frame);
		setTrackbarPos("Frame", "Source Video",realtime_frame);
		cap >> imageBGR;                            //从相机捕获一帧图像
		realtime_frame = cap.get(CV_CAP_PROP_POS_FRAMES);
		if (cap.get(CV_CAP_PROP_POS_FRAMES) == FramesTotal)
		{
			realtime_frame = 0;
			runstill = 0;
		}
		resize(imageBGR, imageBGR, dsize, 0.0, 0.0, INTER_NEAREST);          //对捕捉的图像进行缩放操作
		imshow("Source Video", imageBGR);
		...
		imshow("Multiline Video", roadimage1);
		...
		imshow("Canny Video", frame[1]);
		imshow("ROI Video", roadimage2);
...
/* 		findContours(roadimage2, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE);
		...
		imshow("Multiline Video", imageBGR); */
