	...
	while(true)
	{
		...
		while (1)
		{
			...
			if(receivemsg(connfd, controlmsg)==-1)
				break;
			cout << "Angle:" << (int)(controlmsg[0]) << "	Speed:" << ((int)(controlmsg[1]) +(int)(controlmsg[2])*256) << "\n";
			//DWORD clk2 = GetTickCount();
			//cout << "Go and Return time: " << clk2 - clk1 << " ms" << "	\n";
			//Sleep(28);
			SpeedAndAngle.linear.x=(int)(controlmsg[1]) +(int)(controlmsg[2])*256;
			SpeedAndAngle.angular.z=(int)(controlmsg[0]);
